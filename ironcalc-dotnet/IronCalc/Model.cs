...
public class Model : IDisposable
{
    ...
    public static Model NewEmpty(string name, string locale, string timezone, string languageId)
    {
        unsafe
        {
            ...
            var languageIdBytes = Encoding.UTF8.GetBytes(languageId);
            fixed (byte* nameP = nameBytes)
            fixed (byte* localeP = localeBytes)
            fixed (byte* timezoneP = timezoneBytes)
            fixed (byte* languageP = languageIdBytes)
            {
                var ctx = NativeMethods.new_empty(nameP, localeP, timezoneP, languageP);
                if (ctx.is_ok)
                {
    ...
    /// <returns>A new `Model` instance.</returns>
    public static Model LoadFromXlsxBytes(byte[] bytes, string locale, string timezone, string languageId, string? name = null)
    {
        unsafe
        {
            var localeBytes = Encoding.UTF8.GetBytes(locale);
            ...
            {
                var ctx = NativeMethods.load_from_xlsx_bytes(
                    byteP,
                    bytes.Length,
                    localeP,
                    timezoneP,
